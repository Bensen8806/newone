...
const Chip = forwardRef<HTMLButtonElement, any>(function Chip({ mv, children, ...rest }, ref) {
...
export default function JellyRadio({
  ...
  defaultValue,
  onChange,
  chipColor = 'rgba(18, 15, 23, 0.6)',
  activeColor = '#ffffff',
  textColor = 'rgba(255, 255, 255, 0.7)',
  activeTextColor = '#120F17',
  size = 'md',
  gap = 12,
  radius = 24,
  swell = 0.2,
  ...
  const list = items.map((it) => (typeof it === 'string' ? { value: it, label: it } : it)) as JellyRadioItem[];
  ...
  useEffect(() => {
    ...
    applied.current = at;
    apply(at, false); // changed to false to animate when value changes from outside
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [at]);
